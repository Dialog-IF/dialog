...
#include "abbrevs.inc" // This one is weird: it's definitions, not just declarations, but it's meant to be incorporated into this .c file
// The only reason it gets its own file is because it's automatically generated by a tool at the moment
#define MAX_ABBREVS 96
...
// There's no good way to calculate how much space the abbreviations will need without actually encoding them. So, we use the dryrun flag to mean "don't write anything yet, just track how much space is needed".
uint16_t init_abbrev(uint16_t addr_abbrevstr, int dryrun) {
	uint8_t pentets[MAXSTRING*3];
	uint16_t words[MAXSTRING];
	int i, j, n;
	uint16_t addr_current; // Tracking as we go
	uint16_t addr_string[MAX_ABBREVS]; // Address for each abbreviation string
	char *abbrev;
	
	addr_current = addr_abbrevstr;
	
	// First, encode all the strings
	for(i = 0; i < MAX_ABBREVS; i++) {
		addr_string[i] = addr_current;
	...
	//	report(LVL_WARN, 0, "\tNeeded %d words", n);
		if(!dryrun) {
			for(j = 0; j < n; j++) {
				zcore[addr_current + 2*j + 0] = words[j] >> 8;
				zcore[addr_current + 2*j + 1] = words[j] & 0xff;
			}
		}
	...
	// Next, embed the addresses of all of them
	if(!dryrun) {
		for(i = 0; i < MAX_ABBREVS; i++) { // Address is stored divided by 2
			zcore[addr_current + 2*i + 0] = addr_string[i] >> 9;
			zcore[addr_current + 2*i + 1] = (addr_string[i] >> 1) & 0xff;
		}
	}
	
//	report(LVL_WARN, 0, "Used %x bytes total", addr_current-addr_abbrevstr);
//	report(LVL_WARN, 0, dryrun?"This was a dry run.":"This was a real run!");
	return addr_current - addr_abbrevstr; // How much space was used for the strings (not including the addresses)
}
	...
	while(rsp) {
		...
		for(i = 0; i < cr->ninstr; i++) {
			...
			case I_IF_GVAR_EQ:
				...
				break;
			case I_IF_HAVE_UNDO: // Flags 2 bit 4
				zi = append_instr(r, Z_LOADB);
				zi->oper[0] = SMALL(0);
			...
			case I_IF_HAVE_QUIT:
				...
				break;
			case I_IF_SCRIPT_ACTIVE: // Flags 2 bit 0
				zi = append_instr(r, Z_LOADB);
				zi->oper[0] = SMALL(0);
				zi->oper[1] = SMALL(0x11);
				zi->store = REG_TEMP;
	...
	struct arena *arena)
{
	...
	addr_abbrevstr = org;
	used_abbrevs = init_abbrev(org, 1); // Dry run just to calculate space
	org += used_abbrevs;
	
	addr_abbrevtable = org;
	used_abbrevs += MAX_ABBREVS*2;
	org += MAX_ABBREVS*2;
...
//	report(LVL_DEBUG, 0, "Strings done:   $%06x", org);
	...
	init_abbrev(addr_abbrevstr, 0); // Now actually fill the space we made earlier
	...
	report(LVL_DEBUG, 0, "        Abbreviations:   %5d", used_abbrevs);
