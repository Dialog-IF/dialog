...
Dictionary words are case insensitive. On all backends, `@BiZzArE` is equivalent to `@bizarre`, and `@ΛΟΓΟΣ` equivalent to `@λογοσ`. This is a simple one-to-one replacement, though: it does not recognize alternate letter forms, so `@λογοσ` is _not_ equivalent to `@λογος`, or `@groß` to `@GROSS`. These equivalencies must be handled by the author.
