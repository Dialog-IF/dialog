...
Non-alphabetical characters require multiple slots of storage in the Z-machine, so
that e.g. `@north-east` is considered the same as `@north-ea`, and `@soupçon` the
same as `@soupço`.

This can pose a problem in non-English languages: a Greek-speaking author is likely to be disappointed that `@αυτοκίνητο` ("car") is equivalent to `@αυτός` ("this"), `@αυγή` ("dawn"), and even just `@αυ`.
In cases like this, the `--optimize-alphabet` command-line option can redefine which characters are "alphabetical", prioritizing the ones that appear most commonly in dictionary words.
...
Dictionary words are case insensitive: `@BiZzArE` is equivalent to `@bizarre`, and `@ΛΟΓΟΣ` equivalent to `@λογοσ`. This is a simple one-to-one replacement, though: it does not recognize alternate letter forms, so `@λογοσ` is _not_ equivalent to `@λογος`, or `@groß` to `@GROSS`. These equivalencies must be handled by the author.
