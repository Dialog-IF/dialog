...
Immediately terminates the program. If the program is being run under the 
debugger, the exit status for the debugger will be set to `$ExitStatus`. If the
program is compiled and run outside the debugger, `$ExitStatus` is ignored.
...
Succeeds if the transcript feature is currently active. Since transcripting
continues across a `(restart)`, `(restore)`, or `(restore undo)`, this might
succeed even though `(transcript on)` was never called, or fail even though
`(transcript on)` previously succeeded.
...
adjusted by passing command-line options to the compiler. During debugging and
