...
The _location_ may be followed by one or more _options_, separated from the filename by a comma. For example:
...
* `channel` followed by a colon, optional whitespace, then one or more alphanumeric characters specifies which channel to play the sound on. A different audio resource played on the same channel will replace the old one after a brief cross-fade. Playing a resource while the same resource is already playing will not restart it, so it is safe to embed your background music every turn. If not specified, the default channel is `main`.
...
can refer to them. Any options provided will be embedded verbatim into the CSS.
...
That can now be used in style classes.
...
Note that no `(embed resource $)` is necessary.
