...
The Z-machine backend, the 8-bit Å-machine interpreter, and the interactive
debugger only support a limited number of style attributes. For divs and spans
in the main text area, they are:
...
-iftf-reverse-video::
Supported values are `reverse`, `none`, and `inherit`. This is not standard CSS, and will not be recognized by the web interpreter; some platforms, like the Z-machine and the Apple II, have a special ability to swap their foreground and background colors, and this property was invented to represent that.

CAUTION: Previous versions of Dialog used `text-decoration: reverse` for this purpose. This caused problems with other values of `text-decoration` on the web interpreter and has been removed.
...
The Z-machine and debugger also support two additional properties that the
8-bit Å-machine interpreter does not:

color::
Supported values are `initial`, `inherit`, `black`, `red`, `green`, `yellow`,
`blue`, `magenta`, `cyan`, `white`, or a pound sign `#` followed by six
hexadecimal digits. (As usual in Dialog, the pound sign needs to be escaped
with a backslash `\#` to make it text instead of an object name.) Interpreters
are more likely to support the named colours than the hexadecimal ones.

The value `initial` specifies the interpreter's original choice of colour,
before receiving any instructions from the program; there is no way to know
if this is a light or dark colour, so best practice is to set both `color`
and `background-color` to `initial`, or neither of them.

CAUTION: Note the spelling! It must be "color", not "colour".

background-color::
Supported values are the same as for `color`.

Be aware that not all interpreters allow these properties to be combined. A
span (or combination of spans) intended to be both bold and italic may be
displayed as only one or the other—or even as something else, like bright text
and underlining—depending on the interpreter's capabilities.
...
[#styletest]
=== Testing for style support

Since different interpreters support different amounts of styling, Dialog does
offer a limited way to test for this support. 

----
(interpreter supports styling)
----

This predicate will succeed if the interpreter can offer a minimal level of
styling: at the very least, bold and italic spans will look different. They
might be rendered with bright colors instead of bold text, or underlines
instead of italics, and it may not be possible to combine them—but the
interpreter believes that players will be able to tell them apart.
...
This predicate will succeed if the interpreter supports the `color` and
`background-color` properties, for at least the eight basic colours listed
above. The two spellings are equivalent.

----
(interpreter supports text alignment)
----

This predicate will succeed if the interpreter supports the `text-align`
property, controlling how text is positioned within a div.

Keep in mind, though, that these predicates are intended only as guidelines.
Even if an interpreter can produce yellow and green text, for example, a
colour-blind player might not see the difference. And even if the interpreter
provides "bold" and "italic" tags to a screen reader, that screen reader might
not pronounce them any differently. Treat these predicates as an educated guess
rather than a universal truth, and make sure your players can actually tell
your styles apart before you rely on them for crucial information.

[#bodystyle]
=== Body styles

It is also possible to set the "body style" of the story: the style used outside
of any divs and spans. This is a dangerous power, and results may vary from
interpreter to interpreter. Still, adjusting the background colour of the page
can be an effective way to mark a dramatic shift in the game, or just to set an
appropriate mood.
...
The latter is equivalent to setting the body style to an empty style class, and
in fact, that is how it is implemented under the hood. Whether a new body style
applies to existing text depends on the interpreter, so it is safest to
xref:#clear[clear the screen] after changing the body style.

In theory, a body style can use all the same properties as a div or span style,
but in practice, `color` and `background-color` are the most useful. These,
though, come with their own set of caveats. Most Z-machine interpreters only
apply a new background colour after clearing the screen—but some, like
Gargoyle, apply a new background colour at the next `(get key $)` or `(get input 
$)` instead. Gargoyle also applies a body style's `color` to existing text, but
only if that text has no colour of its own (from a div, span, or previous body
style). Other interpreters leave existing text unmodified.

On the Å-machine, only the web interpreter supports body styles. It applies them
to the entire screen (including existing text) immediately, without needing the
screen to be cleared. Setting the `color` and `background-color` works, but is
not ideal: it does not affect UI elements like the menu or transcript, and does
not respect the user's choice of light or dark mode.

For the Å-machine web interpreter specifically, a set of custom properties can
be used instead:
...
These colours are used for all aspects of the web interpreter, from the game
text to the menus and buttons to the transcript window. By default, the `theme-`
properties are various shades of gray, while the `accent-` properties are a
greenish brown.

From these properties, a dozen more are defined, such as `--night-bg` (the page
background in night mode) and `--ui-hover` (interface elements when hovered).
These can also be adjusted in a body style, if (say) you want different link
colours in night mode and day mode, or want progress bars to use the theme
colours instead of the accent colours. For a full list, look at the CSS files for
the interpreter.

These custom properties are not recognized by the other backends.
...
The predicate `(unstyle)` reverts to the default style of the current div or
span, which is roman by default. Note that these predicates are deprecated, and
may be removed in a future version of Dialog; divs and spans should be used
...
On the Z-machine, text inside the top status area is always rendered in a fixed-pitch font. Unless specified otherwise (with `-iftf-reverse-video: none` in the style class), it will also be displayed in reverse video style.
...
To clear only the status areas (top and inline), use `(clear status bar)`.
To do both, clearing everything on the screen, use `(clear all)`.
These predicates may not be queried from within a span, link, or status area.
Such queries will result in a runtime error.

Be aware that on some interpreters, clearing interferes with the player's ability to scroll back and review earlier parts of the play session. Other interpreters might not support clearing at all.
...
For the most part, Dialog tries to present its output as a document: a series of words divided by spaces and breaks, partitioned off into divs and spans. The exact appearance of this document is then left to the interpreter to realize in whichever way is best for a given platform. Finer details, like the position of each character on the screen, are deliberately left unspecified—there might not even be a "screen" at all, if the game is being played through text-to-speech or a messaging platform.

In some cases, though, the limitations of the Z-machine (and the 8-bit Å-machine interpreters) pose a problem for this model. In the Å-machine web interpreter, for example, a status bar with `height: auto` will expand freely to fit its content. But on the Z-machine, the height of a status bar must always be specified before printing into it. In these cases, Dialog allows the author more fine-grained control; on more flexible backends, this additional detail is simply ignored.
...
These unify their parameter with the appropriate dimension, in ch or em units.
In other words, this tells you how many characters you can safely print without
hitting the edge of the current div. (How many "0" characters, specifically,
in the case of a non-monospace font.)
However, note that not all interpreters can provide this information—on a screen
reader or messaging platform, for example, there might not be any screen to
measure! If this happens, the predicate will simply fail.

On the Z-machine backend and the 8-bit Å-machine interpreter, these
predicates are most useful within the top status area, which always has fixed
dimensions and a monospace font. If used anywhere else, they will attempt to 
provide the width and height of the entire screen.
...
are both supported and enabled.
...
and this is the recommended way to work with "feelies" and embedded graphics.

The _location_ may be followed by one or more _options_, separated by commas. An option may be a simple string, or a key and a value separated by a colon. For example:

[source]
----
(define resource @bg) music/bg.ogg, channel: 1, loop
----

Which options are recognized, and what effects they might have, depends on the
backend and interpreter used. Unrecognized options will never be an error:
interpreters are free to use as many or as few as they like.

Currently, only the Å-machine web interpreter uses options at all, primarily for audio files:
* `channel` followed by a value specifies which channel to play the sound on. A different audio resource played on the same channel will replace the old one after a brief cross-fade. Playing a resource while the same resource is already playing will not restart it, so it is safe to play your background music every turn. If not specified, the default channel is `main`.
* `loop` specifies that the audio should repeat forever rather than stopping at the end. A looping sound only ends when another sound is played on the same channel.

But any interpreter is free to make use of options in whatever ways it likes.
...
Only the actual filename (that is, `manual.pdf` rather than `feelies/manual.pdf`) is used as alt-text, not the full path.
Options, if provided, must come first; any commas after a semicolon will be taken as a literal part of the alt-text.
...
Resources could conceivably be any kind of multimedia, including graphics, sound, and animation.
...
only embeds graphics in `png` or `jpeg` format, and audio in `ogg`, `mp3`, or `wav` format.
Other interpreters and backends (Z-machine, debugger) just print the alt-text.
...
On the Å-machine web interpreter specifically (and no other), any font
resources—that is, any resources whose filenames end in .TTF, .OTF, .EOT,
.WOFF, or .WOFF2—will be embedded into the web page so that
xref:#div[style classes]
can refer to them. Any options provided will be embedded into the CSS.

In other words, a resource definition like this:

[source]
----
(define resource @font)
	HighTowerTextMedium.woff2,
	font-family: "Hightower",
	font-weight: "normal"
----

Will be converted into a CSS declaration:
...
@font-face {
	font-family: "Hightower";
	font-weight: "normal";
	src: url("path/to/HighTowerTextMedium.woff2");
}
----

Which can now be used in style classes.

[source]
----
(style class @bodystyle)
	font-family: "Hightower", serif;
----

At present, this happens when the page loads, with no `(embed resource $)`
necessary. This ensures the browser has time to load the font before any Dialog
code is executed, letting it be used right away. But it's good practice to use
`(embed resource $)` anyway: other interpreters may handle this differently.

This is, strictly speaking, not a feature of Dialog at all; Dialog merely
provides the resource to the Å-machine interpreter, which does the real work.
Future versions of the interpreter might allow other resources to be embedded
in the same way, like stylesheets or scripts.
