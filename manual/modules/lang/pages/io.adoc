...
Floating divs can be further subdivided, either horizontally using floating divs, or vertically using ordinary divs (or simply with line breaks). Note that every floating div is placed relative to the top left corner of its parent, regardless of what else has been printed before it. 
