...
hexadecimal digits. Interpreters are more likely to support the named colors
than the hexadecimal ones. The value `initial` specifies the interpreter's
original choice of color, before receiving any instructions from the program;
there is no way to know if this is a light or dark color, so best practice is
...
of any divs and spans. This is a dangerous power, and is less widely supported
than ordinary divs and spans; results may vary from interpreter to interpreter.
Still, adjusting the background color of the page can be an effective way to
mark a dramatic shift in the game, or just to set a more interesting mood than
...
apply a new background color after clearing the screen---but some, like
Gargoyle, apply a new background color at the next `(get key $)` or `(get input 
$)` instead. Gargoyle also applies a body style's `color` to existing text, but
only if that text has no color of its own (from a div, span, or previous body
...
to the entire screen (including existing text) immediately, without needing the screen to be cleared. Setting the `color` and `background-color` works, but is
...
These properties are used for all aspects of the web interpreter, from the game
...
colors in night mode and day mode, or want progress bars to use the theme
colors instead of the accent colors. For a full list, look at the CSS files for
the interpreter.
