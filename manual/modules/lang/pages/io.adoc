...
Which options are recognized, and what effects they might have, depends on the backend and interpreter used. At present, no interpreters make use of them. However, unrecognized options will never be an error: interpreters are free to use as many or as few as they like.
...
Only the actual filename (that is, `manual.pdf` rather than `feelies/manual.pdf`) is used as alt-text, not the full path. Options, if provided, must come first; any commas after a semicolon will be taken as a literal part of the alt-text.
