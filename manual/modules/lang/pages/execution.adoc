...
When you call one of the spacing predicates, `(no space)`, `(nbsp)`, `(space)`, `(line)`, and `(par)`, Dialog doesn’t display anything immediately. Instead, the next time a word needs to be printed, Dialog inserts _the largest space that’s been requested since the last word_. This means `(no space) (space) (nbsp) (no space) (nbsp)` is equivalent to a single `(space)`, and `(par) (line) (line) (par) (space) (space)` is equivalent to a single `(par)`.

The same goes for xref:io.adoc#div[div margins], which will be introduced in a later chapter. A `(par)` before a div with a one-line margin will still produce only a single blank line.

(The rarely-needed `(nbsp)` creates a “non-breaking space”, which looks identical to a space, but will not allow a line break at that location. It’s mainly used between numbers and units, like “100 km” or “section 5”, and in various non-English languages, such as before certain punctuation marks in French. If an interpreter cannot provide this, it will produce a normal space.)
...
Object names are case sensitive, and may contain alphanumeric characters, plus (`+`), minus (`-`), less than (`<`), greater than (`>`), and underscore (`_`). At present, they can also contain any non-ASCII characters—that is, all Unicode characters with codepoints above U+007F—except the non-breaking space, and any characters escaped by a backslash (`\\`). This may change in the future, though, so please use this for reasonable names like `+#façade+`, `+#äußern+`, or `+#ラーメン+`, rather than `+#\\␣+` (a single escaped space) or `+#a⍽b⍽c+` (multiple words separated by figure spaces). The latter may become invalid in a future version of Dialog.
