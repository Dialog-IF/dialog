...
xref:choicepoints.adoc#at-most-once[(at most once) _statement_] ::

Once the inner statement succeeds, all choice points created since the
`(at most once)` are discarded.
...
xref:beyondprg.adoc#interface[(interface (name of predicate))]::
...
xref:io.adoc#bodystyle[(body style $Class)]::

Applies a style class to the main body of output.
...
xref:beyondprg.adoc#metadata[(compiler version)]::
...
xref:beyondprg.adoc#runtimeerror[(display memory statistics)]::
...
xref:io.adoc#styletest[(interpreter supports color)]::

Succeeds if the current interpreter supports changing the colour of the text
and background.

xref:io.adoc#styletest[(interpreter supports colour)]::

Alternate spelling of `(interpreter supports color)`.
...
xref:io.adoc#styletest[(interpreter supports styling)]::

Succeeds if the current interpreter supports basic styling. At the very least,
this means bold and italic spans will look different from normal text, and from
each other.

xref:io.adoc#styletest[(interpreter supports text alignment)]::

Succeeds if the current interpreter can change the alignment of text: centering
it or aligning it to the right margin.
...
xref:builtins.adoc#system[(quit $Status)]::

As `(quit)`, but also sets the exit status of the interactive debugger.
On Z-machine or Å-machine, it is exactly equivalent to `(quit)`.
...
xref:io.adoc#bodystyle[(reset body style)]::

Undoes the effects of `(body style $)`.
...
xref:beyondprg.adoc#metadata[(serial number)]::
...
xref:builtins.adoc#system[(transcript active)]::

Tests if transcription is currently active.
...
xref:beyondprg.adoc#runtimeerror[(error $ErrorCode entry point)]::
...
xref:beyondprg.adoc#metadata[(story author)]::

Metadata: Defines the author of the story.

xref:beyondprg.adoc#metadata[(story blurb)]::

Metadata: Defines the blurb for the story.

xref:beyondprg.adoc#metadata[(story ifid)]::

Metadata: Defines the IFID of the story.

xref:beyondprg.adoc#metadata[(story noun)]::

Metadata: Defines the noun (also known as the headline) of the story.

xref:beyondprg.adoc#metadata[(story release $N)]::

Metadata: Defines the release number of the story.

xref:beyondprg.adoc#metadata[(story title)]::

Metadata: Defines the title of the story.

xref:beyondprg.adoc#metadata[(library version)]::
