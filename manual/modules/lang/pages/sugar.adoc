...
Topics can also provide a limited sort of error-checking. Because
xref:execution.adoc#thinobjects[Dialog objects are thin],
it is very easy to create them by accident: if you mistype `#apple` as `#aple`
even a single time, Dialog will happily create an `#aple` alongside your
`#apple`, leading to hard-to-find bugs. As a safeguard, the compiler can
optionally produce a warning for any object that never appears as a topic.
By default, these warnings are enabled for any program that includes the
library; but they can always be enabled or disabled on the command line.
...
as `*(fruit $F)` can be used to backtrack over every fruit in the game.
...
[source]
----
\x{28}\x{70}\x{72}\x{6f}\x{67}\x{72}\x{61}\x{6d}\x{20}\x{65}\x{6e}\x{74}\x{72}\x{79}\x{20}\x{70}\x{6f}\x{69}\x{6e}\x{74}\x{29}
----
...
a star and some unique number, since these objects have no source-code names.
