...
The default rule for `(on every tick)` makes a multi-query to
...
It's sometimes useful to `(stop)` all further actions in an `(on every tick)`
rule. For example, an `(on every tick in #dungeon)` rule might trigger an
elaborate cutscene and whisk the player off to another room entirely. After
that, they will probably want to revise their next actions!

Using `(stop)` in this context can lead to strange bugs, though, since the
`(tick)` was cut off halfway through. A better alternative is
`(stop after this tick)`, which allows the current tick to finish, then
`(stop)`s immediately after.
...
advanced. The default implementation of `refuse` checks whether the objects
mentioned in the command are present and within reach of the player.
If they're not, `refuse` prints a message about it and succeeds, just like
a `prevent` rule. The reason for having two different rules (refuse and
prevent), is that it's generally a good idea to check for reachability first.
The action-specific prevent-rules are then free to phrase their failure
messages in a way that presupposes reachability (e.g. “the door is locked”,
which you wouldn't know if you couldn't reach it).

By default, `refuse` assumes that every object involved in the action, except
for directions and relations, must be present; and every object that must be
present, must also be within reach. But for some actions, this is too strict a
requirement. It's perfectly reasonable to `[examine #sky]` without needing to
touch it. In previous editions of Dialog, this was handled by modifying `refuse`
directly.

Since version 1.1.0, though, the library provides more specific predicates for
this purpose. This is the default implementation of `(refuse $)`:
...
If a particular action does not require touch, the author can add a
xref:lang:control.adoc#negation[negated rule definition].
For the case of examining:
...
Similarly, some actions don't require their objects to be present:
...
Note the xref:lang:choicepoints.adoc#just[`(just)` keyword], which turns
off the default rule that is defined later in the source code.

And conversely, new `refuse` rules can add extra checks to a particular action:
...
The above code queries a _when-predicate_; these check for common error
conditions and print appropriate messages. The full set of when-predicates is
documented in xref:miscfeat.adoc#commoncomplaints[Chapter 11].

When a story overrides a `refuse` requirement, the parameter is often bound to a
...
[source]
----
~(action [drink #cloud] requires #cloud to be touched)
...
`*(before $)` {multi-query}, so there can be several before-rules for any
given action.
...
to _..._)” message, before querying `(try $)`, and then
...
is triggered automatically by the game. And if any of these actions should fail
(if the door can't be unlocked, say), xref:tickstop[`(stop)`] will automatically
cut short the entire sequence.
