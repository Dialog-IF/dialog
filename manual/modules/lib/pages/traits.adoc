...
Technically, `(a $)` and `(the $)` don't directly call `(name $)`. Instead, they
call an intermediate predicate `(tagged name $)`, which calls `(name $)`.

Most authors will never need to work with `(tagged name $)`, and this extra
layer of indirection will be optimized away by the compiler. Sometimes, though,
it is useful to attach extra words to the beginning or end of an object's name,
for an entire category of objects:
...
(open container $Obj)
	...
	~(room $Obj)
	($Obj is open)
(tagged name (open container $Obj))
	open (name $Obj)
(an (open container $Obj))
```
