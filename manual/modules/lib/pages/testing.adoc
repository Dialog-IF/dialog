...
take longer to compile and execute than unit tests do. Having both
kinds of tests is important, as each style can find problems that are
difficult for the other kind to turn up.
...
that path. But many games have multiple incompatible alternatives,
multiple endings, and so forth. You can write additional tests to test
the additional paths, but manually managing those can be awkward.

A solution to that problem is Howard Ship's `dgt` tool, which includes
a feature called the _Skein_. The Skein allows you to interactively
walk through your game through multiple branching paths, and "bless"
each branch's output when it's correct. You can then re-run the Skein
after making changes to your game, and confirm that you didn't break
anything. 
