...
[#diff]
=== Simple Tests with `aamrun`, `dfrotz`, and `diff`

There are several ways to do interpreter-based testing in Dialog
...
An exciting alternative to `regtest.py` or `diff` is Howard Ship's
`dgt` tool, which includes a feature called the _Skein_. The Skein
allows you to interactively walk through your game through multiple
branching paths, and "bless" your game's output when it's correct. You
can then re-run the Skein after making changes to your game, and
confirm that you didn't break anything.
