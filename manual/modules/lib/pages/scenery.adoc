...
using xref:lang:execution.adoc#query[predicates]
...
names (`#table`, `#chair`), printed names (“#wooden
table”, “chair”), and descriptions to be revealed by the
...
xref:lang:execution.adoc#query[queried] from within
rule bodies, like so:
...
With a
xref:lang:choicepoints.adoc#multi-query[multi-query], you can
...
Back to xref:intro.adoc[] or onwards to TRAITS.
