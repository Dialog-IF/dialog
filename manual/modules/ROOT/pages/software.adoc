...
Versions of Dialog from before 2025 can be found here:

{old-home-page}

Versions from 2025 onward can be found here:
...
and then use the command `brew install dialog-if/brew/dialog`, which will
install `dgdebug` and `dialogc`. This command also installs Dialog's standard libraries (`stdlib.dg` and `stddebug.dg`) to
either `/opt/homebrew/share/dialog-if` or `/usr/local/share/dialog-if`
(for either ARM-based Macs or Intel-based Macs, respectively)
footnote:[The command `brew --prefix dialog-if` will identify
an installation directory; the libraries are nested at `share/dialog-if`
and a copy of this manual is at `share/doc/dialog-if/index.html`].

Both the compiler and the interactive debugger have to be started from a
command-line shell. Depending on what operating system you use, you may have to
...
Dialog is a compiled language. Source code is stored in text files with the filename extension `.dg`, which must be encoded in UTF-8.
The compiler delivers its output in `.zblorb`, `.z8`, `.z5`, or `.aastory` format.
When the zblorb format is chosen, the compiler can optionally include cover art with the story,
but this feature is not regarded as a part of the Dialog language itself.

image:overview.png[overview]

To compile a Dialog story into `z8` format, invoke the compiler with
...
This will produce a file called `story.z8`, based on the name of the
first source code file.

=== Producing stories for the Z-machine

By default, the compiler uses the `z8` output format, producing story files for
version 8 of the Z-machine. Adding `-t z5` to the command line will instead make
a Z-machine version 5 file, limited to 256 KiB rather than 512 KiB for `z8`.
However, the `z5` format is more compact, and switching to this format can reduce
the size of the story file by about 5%.

Raw Z-code files are quite limited: they only contain the story itself, like a
paper manuscript of a book. `-t zblorb` switches to the `zblorb` output format,
which is a `z8` file packaged with extra bibliographic information: the 
metaphorical cover, title page, and other details wrapped around a published
book. This is generally better for a proper release, but it requires you to
include those various details, making it less good for the brief examples in the
manual.

The examples in Part I do not make use of the standard library at all, so they
are compiled like this:

`dialogc -t z5 example.dg`
...
the Commodore 64, using the tool `aambundle` from the Å-machine release archive
at {aa-home-page} (formerly {aa-old-home-page}).
...
Only one of the `js` files in that directory contains the story itself; the rest
provide all the interpreter machinery to run it. To rebuild only that one `js`
file without overwriting any changes you've made to the others, use:

[role=output]
```
aambundle -t web:story -o my_game/resources/story.js example.aastory
```
...
dump additional information that can be useful for debugging. Give it
a third time `-vvv` to see the full code in intermediate representation.
...
To debug your game, launch the debugger from the command line with your source
...
The debugger also accepts a couple of command-line options. For a complete list,
type:
...
Shows the current state of the object tree (that is, the `($ has parent $)`
per-object variable).

@dynamic::

Shows the current state of all dynamic predicates (global flags, global variables,
...
while debugging. These statements will be completely ignored when compiling
for Z-machine or Å-machine.
